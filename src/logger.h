...
#define _log_message_backup(priority, priority_str, color, fmt, args...) \
do {									\
    struct timeval tv; \
...
do {									\
    ...
    sprintf(szNow, "%04d-%02d-%02d %02d:%02d:%02d.%03d", \
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, \
            tm.tm_hour, tm.tm_min, tm.tm_sec, msec); \
    log_write(priority, "%s " color "| " priority_str " |\033[0m " fmt color " | \033[0m %s:%s() Line %d\n", szNow, ##args, __FILE__, __func__, __LINE__);	\
    if (likely(is_daemon)) \
    log_flush(); \
} while (0)
...
extern int init_logger(const char* program_name, int log_level);
