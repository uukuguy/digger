...
def cmd_import_samples(args):
    if not args.corpus_dir is None:
        aa.set_arg('corpus_dir' , args.corpus_dir)
    if not args.samples_name is None:
        aa.set_arg('samples_name' , args.samples_name)
    if not args.xls_file is None:
        aa.set_arg('xls_file' , args.xls_file)

    corpus_dir = aa.get_arg('global', 'corpus_dir')
    samples_name = aa.get_arg('global', 'samples_name')
    xls_file = aa.get_arg('global', 'xls_file')

    do_import_samples(corpus_dir, samples_name, xls_file)


# ---------------- cmd_rebuild() ----------------
def cmd_rebuild(aa, args):
    if not args.corpus_dir is None:
        aa.set_arg('corpus_dir' , args.corpus_dir)
    if not args.samples_name is None:
        aa.set_arg('samples_name' , args.samples_name)
...
# ---------------- cmd_export_samples() ----------------
def cmd_export_samples(aa, args):
    if not args.corpus_dir is None:
        aa.set_arg('corpus_dir' , args.corpus_dir)
    if not args.samples_name is None:
        aa.set_arg('samples_name' , args.samples_name)
...
# ---------------- cmd_show() ----------------
def cmd_show(aa, args):
    if not args.corpus_dir is None:
        aa.corpus_dir = args.corpus_dir
    if not args.samples_name is None:
        aa.samples_name = args.samples_name
...
def main():
    ...
    parser_import_samples.add_argument('--corpus_dir', type=str, help='Corpus root dir.')
    parser_import_samples.add_argument('--samples_name', type=str, help='The samples\'s name in corpus.')
    ...
    parser_rebuild.add_argument('--corpus_dir', type=str, help='Corpus root dir.')
    parser_rebuild.add_argument('--samples_name', type=str, help='The samples\'s name in corpus.')
    ...
    parser_export_samples.add_argument('--corpus_dir', type=str, help='Corpus root dir.')
    parser_export_samples.add_argument('--samples_name', type=str, help='The samples\'s name in corpus.')
    ...
    parser_show.add_argument('--corpus_dir', type=str, help='Corpus root dir.')
    parser_show.add_argument('--samples_name', type=str, help='The samples\'s name in corpus.')
